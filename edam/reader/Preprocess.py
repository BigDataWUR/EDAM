...
class Preprocess(object):
    ...
    def __init__(self, input_file, template_file):
        ...
        """
        self.preprocess_logger = logging.getLogger(
            'edam.reader.Preprocess.Preprocess')
        ...
        if self.header != "":
            ...
            self.new_input_file_data, self.new_input_file_preamble = self.split_preamble_from_data(
                self.template_file, self.input_file)

    def split_preamble_from_data(self, template_file, input_file):
        # eg. py, pdf
        # Consider a header which contains placeholders
        # e.g "Speed of maximum wind gust (km/h)","Time of maximum wind
        # gust",{{temp.timestamp.hour=9}}
        ...
        # 2. input_header which is similar to template_header (in place of the
        # placeholder we have something different
        ...
        for line in input_text.readlines():
            ...
            if header_without_placeholders in cleaned_line or header_without_placeholders in \
        ...
        template_lines_before, _, template_lines_after = template_file_text.partition(
            self.template_header)
        input_lines_text_before, _, input_lines_text_after = input_file_text.partition(
            self.input_header)
        # TODO: I should probably insert an else statement here.
        # TODO: If there is no match, I could just rename the file to
        # sth_data.sth
        ...
        if template_lines_before != "":
            ...
            template_data_text = self.header + template_lines_after.replace(template_preamble_text,
                                                                            '')
            input_data_text = self.header + \
                              input_lines_text_after.replace(input_preamble_text, '')
