...
from edam.reader.models import Station, Sensors, AbstractObservables, UnitsOfMeasurement, \
    HelperTemplateIDs
from edam.utilities.utilities import parse_for_iterations, extract_data_from_preamble, \
...
class SourceConfiguration:
    ...
    def handler(self):
        ...
        # If these fields are empty, it means that they should default to
        # "unknown values"
        self.set_units_of_measurement()
        self.set_sensors()

        # Update dataframe. For some reason int is transformed to float. So
        # here I revert this (for the affected columns
        self.helper_template['station_id'] = self.helper_template['station_id'].apply(
            int)
        self.helper_template['sensor_id'] = self.helper_template['sensor_id'].apply(
            int)
        self.helper_template['abstract_observable_id'] = self.helper_template[
            'abstract_observable_id'].apply(int)
        self.helper_template['unit_id'] = self.helper_template['unit_id'].apply(
            int)
        ...
        # We want to pass the first station_id since we already have this id
        # incorporated.
        temp = self.helper_template.copy(deep=True)
        for station_id in self.station_id[1:]:
            temp['station_id'] = station_id
            self.helper_template = self.helper_template.append(
                temp, ignore_index=True)

        del temp
        # We have to check if we have duplicates!
        df_cleaned = self.database.clean_df_db_dups(df=self.helper_template,
                                                    tablename='HelperTemplateIDs',
                                                    dup_cols=list(self.helper_template))

        self.database.__add_dataframe__(dataframe=df_cleaned, table='HelperTemplateIDs',
                                        index=False)
    ...
    def set_station(self, parse_from_yaml):
        ...
        if self.input_preamble.seek(0, os.SEEK_END) > 0 and self.template_preamble.seek(0,
                                                                                        os.SEEK_END) > 0:
            self.input_preamble.seek(0)
            self.template_preamble.seek(0)
            station = extract_data_from_preamble(
                station,
                preamble_template=self.template_preamble,
        ...
        exists, station_from_db = self.database.__check_station_is_in_db__(
            station)
        if exists:
    ...
    def set_observables(self, iter_or_not=None):
        ...
        for obs in observables:
            ...
            if exists:
                ...
                _, respective_abstract_observable_id = self.database.__add_item__(
                    observable)
    ...
    def set_units_of_measurement(self):
        ...
        if self.content['Units of Measurement'] is None:
            ...
            for uom in self.content['Units of Measurement']:
                # type: dict
                uom_as_dict = self.content['Units of Measurement'][uom]
                if uom_as_dict['relevant_observables'] == '':
                    relevant_observables = self.all_observable_ids
                else:
                    relevant_observables = uom_as_dict['relevant_observables'].split(
                        ',')  # type: list
                    # remove spaces
        ...
        if self.content['Sensors'] is None:
            ...
            for observable_observable_id in relevant_observables:
                sensor = self.update_sensor(
                    observable_observable_id, None, default_empty_sensor)

                exists, sensor_id = self.database.__check_sensor_is_in_db__(
                    sensor)
            ...
            for sensor in self.content['Sensors']:
                ...
                if sensor_as_dict['relevant_observables'] == '':
                    ...
                    relevant_observables = sensor_as_dict['relevant_observables'].split(
                        ',')  # type: list
                    relevant_observables = list(
                        map(str.strip, relevant_observables))
                    # No need to keep it any more
                # No need to keep it any more
                ...
                # After retrieving this id, we will update 'sensor' object,
                # check if it's in db already and finally store it.

                # A generic sensor can have more than one relevant_observables
                # In this case, we are going to create as many generic sensor
                # objects as the relevant_observables..

                # We are going to determine the observable_id through the
                # observable_id

                for observable_observable_id in relevant_observables:
                    sensor = self.update_sensor(
                        observable_observable_id, sensor, sensor_as_dict)
                    sensor.generic = True
                    exists, sensor_id = self.database.__check_sensor_is_in_db__(
                        sensor)
                    # Next line is resolving a bug introduced by sqlite
                    # For some unknown reason a str type was interpreted as dict when it
                    # came for storing
    ...
    def update_sensor(self, observable_observable_id, sensor, sensor_as_dict):
        ...
        abstract_observable_id = \
            self.helper_template.loc[(self.helper_template['station_id'] == self.station_id[0]) &
                                     (self.helper_template[
                                          'observable_id'] == observable_observable_id)][
    ...
    def __check_type_of_field(self, field_name):
        ...
        try:
            if ('source' and 'template') in self.content[field_name]:
                ...
                sexists, _, sourcef, source_io_object = handle_input_parameter(
                    source)
                texists, _, templatef, template_io_object = handle_input_parameter(
                    template)
            ...
            # That is because metadata are PROBABLY stored in input files
            # (preamble)
